...
import {calcDevisTotals, type DevisDraft} from '@/lib/docDraft';
    ...
    <div className="print-sheet rounded-2xl border bg-white p-6">
      <div className="flex items-start justify-between gap-4">
        <div>
          <div className="text-xl font-bold">
            {template === 'modern' ? 'DEVIS' : 'Devis'}
          </div>
          <div className="text-sm opacity-70">
            {draft.number} • {draft.dateISO}
          </div>
        </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {items.map((it) => {
              ...
              const tvaRate = Number.isFinite(it.tvaRate) ? it.tvaRate : 0;
              ...
              return (
                <tr key={it.lineId} className="border-t">
                  ...
                  <td className="p-2 text-right">{formatMoney(locale, unitPrice)}</td>
                  <td className="p-2 text-right">{tvaRate}%</td>
      ...
      <div className="mt-6 ml-auto w-full max-w-sm space-y-2 text-sm">
        <div className="flex justify-between">
          <span>Subtotal</span>
          <span>{formatMoney(locale, totals.subtotal)}</span>
        </div>
        <div className="flex justify-between">
          <span>TVA</span>
          <span>{formatMoney(locale, totals.tvaTotal)}</span>
        </div>
        {totals.discountAmount > 0 ? (
          <div className="flex justify-between">
            <span>Discount</span>
            <span>-{formatMoney(locale, totals.discountAmount)}</span>
          </div>
        ) : null}
        <div className="flex justify-between text-base font-bold">
          <span>Total</span>
          <span>{formatMoney(locale, totals.total)}</span>
        </div>
      </div>
